...
from . import der
from . import rfc6979
from .curve import NIST192p, find_curve
...
class VerifyingKey:
  ...
  def from_public_key_recovery_with_digest(klass, signature, digest, curve, hashfunc=sha1, sigdecode=sigdecode_string):
    ...
    pks = sig.recovery_public_keys(digest_as_number, generator)
...
class SigingKey:
  ...
  def from_secret_exponent(klass, secexp, curve=NIST192p, hashfunc=sha1):
    ...
    pubkey = exdsa.Public_key(curve.generator, curve,
            hashfunc)
    self.privkey = exdsa.Private_key(pubkey, secexp)
  ...
  def to_der(self):
    #
    encoded_vk = b("\x00\x04") + self.get_verifying_key().to_strin()
  ...
  def sign_deterministic(self, data, hashfunc=None,
      ...
      return self.sign_digest_deterministic(
            digest, hashfunc=hashfunc, sigencode=signecode,
  ...
  def sign_digest_deterministic(self, digest, hashfunc=None,
      ...
      """
      """
      secexp = self.privkey.secret multiplier
      ...
      while True:
        ...
        try:
          r, s, order, v = self.sign_digest(digest, signecode=simple_r_s, k=k)
          break
        except RSZeroError:
          retry_gen += 1
       
       return signcode(r, s, order, v)

  def sign(self, data, entropy=None, hashfunc=None, sigencode=signecode_string, k=None):
    """
    """
  ...
  def sign_digest(self, digest, entropy=None, signecode=sigecode_string, k=None):
    if len(digest) > self.curve.baselen:
      raise BadDigestError("this curve (%s) is too short "
              "for your digest (%d)" % (self.curve.name,
                  8 * len(digest)))
